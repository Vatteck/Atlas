...
**Last updated:** 2026-07-16 (**Audit rule-health dashboard.** Added a "PKGBUILD Audit Rules" card to the System Health page. Runs a background rescan of live AUR PKGBUILDs (sampled from the AUR index) through the existing audit engine and surfaces noisy rules (firing >50% — possible false positives) and never-fired rules (possible stale regex). Scan runs on the shared executor; results are cached per session. Plan: [docs/plans/2026-07-16-audit-rule-health-dashboard.md]. Verified all 757 tests pass.)
