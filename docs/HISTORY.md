# HISTORY — the archived Done log

> This is the **archive**. It holds the full chronological record of everything Atlas has
> shipped, plus retired gotchas and the historical decision log. It was split out of
> `STATUS.md` on 2026-08-01, when that file had grown to 2,379 lines and stopped working as
> a live baton.
>
> **Agents: you do not need to read this file to start work.** Read
> [STATUS.md](STATUS.md) — it is the live state. Come here only to answer "when/why did we
> ship X?" or "have we tried Y before?". Search it, don't read it top to bottom.
>
> The authoritative release record is [CHANGELOG.md](../CHANGELOG.md); the themed shipped
> index is in [BACKLOG.md](BACKLOG.md).
...
## Retired gotchas (resolved or obsolete — kept so they aren't re-derived)

- **Tray mode was gone, then rebuilt.** The Qt rebrand left `tray.py`/`manage.py` importing the
  deleted `atlas.view.qt.*`, so `atlas-tray` crashed on launch; the broken tray, the `atlas-tray`
  entry point, the `--tray` arg and `context.new_qt_application`/`set_theme` were removed
  (2026-05-29). A non-Qt AppIndicator/SNI tray was later reintroduced — see the live gotcha in
  STATUS.md.
- ~~**Residual PyQt5 coupling**~~ **Removed (2026-06-01).** The webview app no longer imports
  PyQt5: dropped the Qt HDPI block in `app.py`; de-Qt'd `view/util/util.py`
  (`get_default_icon`→`get_default_icon_path`; `restart_app` stops the pywebview/GTK loop via
  `webview.windows[*].destroy()` instead of `QCoreApplication.exit()`); removed the Qt widget-style
  selector + `QApplication`/`QStyleFactory` import from `view/core/settings.py`; swapped `pyqt5` →
  `pywebview` in `pyproject.toml`. **Verified** by importing `atlas.app` and `atlas.cli.app` with
  PyQt5 forced-absent (`sys.modules['PyQt5']=None`). *(Note: `stylesheet.py` — the last Qt-era file
  — was deleted 2026-06-21. A stale PyQt5 may still sit in an old local venv; it is not a
  dependency.)*
- **App run-status (2026-05-29).** ✅ GUI confirmed working (window loads, lazy gem init fires,
  background prepare ~10ms, suggestions run). Log noise at the time was expected: first-run (no
  cache/db), no-root (`pacman -Sy` can't sync), harmless pywebview GTK `window.native.*` warnings,
  and `atlas-files` download failures (which predated the repo existing).
- ~~**`atlas-files` repo content.**~~ Resolved 2026-05-29 — the repo exists and all 10 download
  paths return 200. Content is package-manager-agnostic with no `bauh`/`vinifmor` self-references.
  Suggestion lists were curated for Arch (2026-06-01) and AUR suggestions were wired up via the AUR
  RPC (`list_suggestions` resolves non-repo names through `aur_client.get_info`, one batched call,
  gated on `aur.is_supported`); `arch/suggestions.txt` holds repo + AUR names and `aur/suggestions.txt`
  is unused legacy. Tests: `tests/gems/arch/test_suggestions_aur.py`. Plan:
  [plans/2026-06-01-aur-suggestions.md](plans/2026-06-01-aur-suggestions.md).
- ~~**Silent fallback hides Rust bugs.**~~ Obsolete — the Rust extension was removed 2026-06-01.
  (Historically: native calls routed through `atlas/gems/arch/native.py` with `ATLAS_RS_DEBUG` /
  `ATLAS_DISABLE_RS` switches.)
...
## Historical decision log (2026-05-28 → 2026-05-30 — the Rust/Qt era)
...
- **2026-05-30** — Converted the remaining `window.confirm`/`alert` watcher dialogs
  ...
  resolves). Same root cause as the password fix: WebKitGTK has no native JS dialogs.
- **2026-05-30** — Fixed Arch/AUR "no root access" installs. Root cause: `api.py`
  ...
  `plans/2026-05-30-root-password-flow-design.md`.
...
- **2026-05-29** — Retired the native dependency resolver: removed `resolver.rs`,
  ...
  faster). `lib.rs` became `map_srcinfo` only; `.so` 3.6 MB → 621 KB.
- **2026-05-29** — Reverted the native pacman info parser (~1.2×, marshalling-bound) to
  cut maintenance surface; kept the `_parse_info_output_py` extraction + test.
...
- **2026-05-29** — Wired native pacman `-Si` parser into `map_updates_data` (parity-tested,
  ...
  resolved). Chose the pacman-parser task after finding version-compare is NOT the update
  hot path (pacman -Qu does repo comparison; per-call vercmp across PyO3 would be slower).
- **2026-05-28** — Phase 0 closed with a benchmark (`benchmarks/bench_srcinfo.py`). It
  ...
  pinned `setup.py debug=False` → release builds, native ~2× faster. Lesson encoded:
  always measure release builds.
- **2026-05-28** — Fixed native `deps_data` schema: Rust emitted canonical short keys via
  per-source `to_deps_data()`; pacman.rs parsed Description/Download/Installed sizes.
- **2026-05-28** — Phase 0 instrumentation: all native (`atlas_rs`) calls routed through
